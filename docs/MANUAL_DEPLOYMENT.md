...
> **推奨**: existingRedshift モードの初回デプロイでは `npm run cdk -- deploy --all --no-rollback`
> を検討してください。後述の読み取り専用検証で失敗した場合に、CloudFront など作成済みの
> リソースを巻き戻さずに停止できるため、DB ユーザーの権限を修正して再デプロイする
> サイクルが大幅に短くなります（デフォルトではスタック全体がロールバックします）。

デプロイ中に **読み取り専用検証** (カスタムリソース) が実行されます。
検証は**許可リスト方式**です: 指定した DB ユーザー（およびそのユーザーに付与された
ロール）が持つ権限のうち、**SELECT / USAGE / TEMP 以外のものが 1 つでもあれば**
デプロイは失敗します。具体的には以下をすべて検査します:

- superuser でないこと
- ユーザーへの直接 GRANT（`SHOW GRANTS`。scoped permission 含む）
- ロール経由の GRANT（入れ子ロールも展開して検査。`sys:*` システムロールは違反扱い）
- テーブル / スキーマの**所有権**を持たないこと（owner は GRANT なしで DROP 等が可能なため）
- PUBLIC 経由で継承した書き込み系権限がないこと（`has_table_privilege` による保険チェック）

> **TEMP 権限について**: 一時テーブルの作成はセッション限りで既存データを変更できないため
...
> **既知の制限**: Redshift Spectrum の外部テーブルは Admin UI のテーブル一覧に
> 表示されません（通常テーブルのみ対応。外部テーブル対応は今後の拡張予定です）。

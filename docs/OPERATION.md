...
> `cdk.json` で `csvInputBucketName` に既存バケットを指定してデプロイした場合、ローカルアップロードモードは無効化され、「既存 S3 パス指定」のみ利用できます（既存バケットに CORS 設定を入れない前提のため）。

**モード A: ローカルファイルアップロード**（新規バケット作成時のみ）
...
1. 「既存 S3 パス指定」を選択
2. 入力欄の上に対象の CSV バケット名が表示されるので、そのバケット内の S3 prefix を入力（例: `data/sales/`）
